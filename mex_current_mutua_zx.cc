...
extern "C" {
    ...
    double NewellF2_Int_Y_s(double, double, double, double, double, double);
...
double NewellF2_Int_Y_s(double x, double y, double z, double dx, double dy, double dz)
{
    const int N=5;
    int i;
    double xi[2*N+1];
    ...
    for (i = 0; i <= 2*N; i++) {
        xi[i] = x - (double)i/(2*N)*dy;
        F[i] = Newellf(xi[i],y,z) - Newellf(xi[i],0,z)
              -Newellf(xi[i],y,0) + Newellf(xi[i],0,0);
    }
    ...
    /*#pragma omp parallel for default(shared) private(i) schedule(static,chunk) reduction(+:FE)*/
    for (i = 1; i < 2*N; i+=2) {
        FE = FE + F[i];
    }
    /*#pragma omp parallel for default(shared) private(i) schedule(static,chunk) reduction(+:FO)*/
    for (i = 2; i < 2*N; i+=2) {
        FO = FO + F[i];
    }
        
    double FF = (F[0] + 4*FE + 2*FO + F[2*N])*dy/(6*N);
    ...
    for (k = 0; k < Nz/2; k++) {
        ...
        for (j = 0; j < Ny/2; j++) {
            ...
            for (i = 0; i < Nx/2; i++) {
                ...
                i_mul = Xo+i*dx;
                i_min = Xo-(i+1)*dx;
                out[k_pos+j_pos+i_pos] = NewellF2_Int_Y_s(j_mul,k_mul,i_mul,dx,dy,dz);
                out[k_pos+j_pos+i_inv] = NewellF2_Int_Y_s(j_mul,k_mul,i_min,dx,dy,dz);
                out[k_pos+j_inv+i_pos] = NewellF2_Int_Y_s(j_min,k_mul,i_mul,dx,dy,dz);
                out[k_pos+j_inv+i_inv] = NewellF2_Int_Y_s(j_min,k_mul,i_min,dx,dy,dz);
                out[k_inv+j_pos+i_pos] = NewellF2_Int_Y_s(j_mul,k_min,i_mul,dx,dy,dz);
                out[k_inv+j_pos+i_inv] = NewellF2_Int_Y_s(j_mul,k_min,i_min,dx,dy,dz);
                out[k_inv+j_inv+i_pos] = NewellF2_Int_Y_s(j_min,k_min,i_mul,dx,dy,dz);
                out[k_inv+j_inv+i_inv] = NewellF2_Int_Y_s(j_min,k_min,i_min,dx,dy,dz);
            }
        }
    }
