    ...
    for (k = 0; k < Nz/2; k++) {
        ...
        for (j = 0; j < Ny/2; j++) {
            ...
            for (i = 0; i < Nx/2; i++) {
                ...
                i_mul = Xo+i*dx;
                i_min = Xo-(i+1)*dx;
                out[i_pos+j_pos+k_pos] = NewellF2(j_mul,k_mul,i_mul);
                out[i_inv+j_pos+k_pos] = NewellF2(j_mul,k_mul,i_min);
                out[i_pos+j_inv+k_pos] = NewellF2(j_min,k_mul,i_mul);
                out[i_inv+j_inv+k_pos] = NewellF2(j_min,k_mul,i_min);
                out[i_pos+j_pos+k_inv] = NewellF2(j_mul,k_min,i_mul);
                out[i_inv+j_pos+k_inv] = NewellF2(j_mul,k_min,i_min);
                out[i_pos+j_inv+k_inv] = NewellF2(j_min,k_min,i_mul);
                out[i_inv+j_inv+k_inv] = NewellF2(j_min,k_min,i_min);
            }
        }
    }
